...
def download_segments(
    ...
    total = sum(len(segs) for segs in buckets.values())
    print(f"Downloading {total} segments across {len(buckets)} labels to {output_root}")
    ...
    label_items = sorted(buckets.items())
    if tqdm_installed:
        label_items = tqdm(label_items, desc="labels")

    n_done = 0
    for folder_name, segments in label_items:
        label_dir = output_root / folder_name
        if not tqdm_installed:
            print(f"Label '{folder_name}': downloading {len(segments)} clip(s)")
        for segment in segments:
            download_clip(segment[0], segment[1], segment[2], label_dir, stdout=stdout)
            n_done += 1
...
def main() -> None:
    ...
    download_segments(buckets, args.output)
